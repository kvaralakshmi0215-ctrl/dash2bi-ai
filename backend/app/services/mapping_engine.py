...
VISUAL_TYPE_MAP = {
    ...
    "areaChart": "Power BI Area Chart",
    "scatterChart": "Power BI Scatter Chart",
    "card": "Power BI Card",
...
def build_mappings(dataset_analysis: dict, dashboard_analysis: dict) -> list[dict]:
    ...
    for visual in dashboard_analysis["detected_visuals"]:
        ...
        if candidate_type == "unsupported" or candidate_type not in VISUAL_TYPE_MAP:
            specific_reason = visual.get("unsupported_reason")
            mappings.append({
                ...
                "confidence": 0.0,
                "level": "unsupported",
                "warning": specific_reason or "This visual could not be statically identified as a supported chart type.",
        ...
        if confidence < CONFIDENCE_MIN_THRESHOLD or (candidate_type == "card" and not entry["field"]) \
                or (candidate_type in ("lineChart", "areaChart") and not (entry["y_axis"] and entry["x_axis"])) \
                or (candidate_type == "scatterChart" and not (entry["x_axis"] and entry["y_axis"])) \
        ...
        # (scatter charts plot two raw numeric columns directly and have no aggregated measure)
