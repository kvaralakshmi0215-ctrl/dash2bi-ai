...
VISUAL_TYPE_TO_PBI_VISUAL = {
    ...
    "Power BI Area Chart": "areaChart",
    "Power BI Scatter Chart": "scatterChart",
    "Power BI Card": "card",
...
def _build_visual_json(mapping: dict, position: dict, primary_table: str) -> dict:
    ...
    if pbi_visual in ("lineChart", "areaChart"):
        query_state = {
    ...
    elif pbi_visual == "scatterChart":
        # Scatter plots two raw numeric columns directly (X/Y roles) rather
        # than an aggregated category/value pair.
        query_state = {
            "X": {"projections": [col_ref(mapping.get("x_axis"))]} if mapping.get("x_axis") else {},
            "Y": {"projections": [col_ref(mapping.get("y_axis"))]} if mapping.get("y_axis") else {},
        }
