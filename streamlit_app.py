...
Navigation is deliberately NOT built with st.tabs — only one step's content
is ever constructed per script run, controlled by st.session_state.step.
This avoids relying on st.tabs' handling of widgets/state across switches.
"""
import os
import sys
import tempfile
...
# ---------------------------------------------------------------------------
DEFAULTS = {
    "step": "upload",  # upload -> analyze -> preview -> convert
...
STEP_ORDER = ["upload", "analyze", "preview", "convert"]
STEP_LABELS = {"upload": "1️⃣ Upload", "analyze": "2️⃣ Analyze", "preview": "3️⃣ Preview & Map", "convert": "4️⃣ Convert & Download"}
...
def reset_downstream(from_step: str):
    """Clear anything computed AFTER `from_step`. Uses strict `<` so
    `from_step`'s own just-computed results are preserved."""
    idx = STEP_ORDER.index(from_step)
    if idx < STEP_ORDER.index("analyze"):
        st.session_state.dataset_analysis = None
        st.session_state.dashboard_analysis = None
    if idx < STEP_ORDER.index("preview"):
        st.session_state.mappings = None
        st.session_state.preview_summary = None
    if idx < STEP_ORDER.index("convert"):
...
# ---------------------------------------------------------------------------
# Header + step navigation (plain buttons, not st.tabs)
# ---------------------------------------------------------------------------
st.title("📊 Dash2BI AI")
st.caption(
...
for col, step in zip(nav_cols, STEP_ORDER):
    with col:
        is_current = st.session_state.step == step
        st.button(
            STEP_LABELS[step],
            key=f"nav_{step}",
            on_click=go_to,
...
if current == "upload":
    ...
    if ready:
        ...
        st.button("Continue to Analyze →", on_click=go_to, args=("analyze",), type="primary")
    else:
...
# ---------------------------------------------------------------------------
    ...
    if not (st.session_state.dataset_path and st.session_state.dashboard_path):
        ...
        st.button("← Go to Upload", on_click=go_to, args=("upload",))
    else:
        if st.button("Run analysis", type="primary", key="run_analysis_btn"):
            with st.spinner("Analyzing dataset (pandas) and dashboard (HTML/JS AST)…"):
                try:
                    dataset_result = excel_analyzer.analyze_dataset(st.session_state.dataset_path)
                    dashboard_result = html_analyzer.analyze_dashboard(st.session_state.dashboard_path)
                    st.session_state.dataset_analysis = dataset_result
                    st.session_state.dashboard_analysis = dashboard_result
        ...
        if ds and dash:
            st.button("Continue to Preview & Map →", on_click=go_to, args=("preview",), type="primary")
...
# ---------------------------------------------------------------------------
    ...
    if not (st.session_state.dataset_analysis and st.session_state.dashboard_analysis):
        ...
        st.button("← Go to Analyze", on_click=go_to, args=("analyze",))
    else:
        if st.button("Build mapping & preview", type="primary", key="build_mapping_btn"):
        ...
        if mappings:
            ...
            for m in mappings:
                ...
                with st.expander(f"{icon} {m['title']} — {m['power_bi_type']} ({LEVEL_LABEL[m['level']]})"):
                    ...
                    if m["level"] == "ai_suggested" and all_columns:
                        current_val = m.get("field") or m.get("category") or m.get("x_axis") or ""
                        new_val = st.selectbox(
                            "Confirm or correct the mapped column",
                            options=[""] + all_columns,
                            index=([""] + all_columns).index(current_val) if current_val in all_columns else 0,
            ...
            st.button("Continue to Convert & Download →", on_click=go_to, args=("convert",), type="primary")

# ---------------------------------------------------------------------------
# Step: Convert & Download
# ---------------------------------------------------------------------------
elif current == "convert":
    if not st.session_state.mappings:
        st.warning("Build the mapping first.")
        st.button("← Go to Preview & Map", on_click=go_to, args=("preview",))
    else:
        ...
        if st.button("Generate Power BI Project", type="primary", key="generate_btn"):
            steps = [
        ...
        if report:
            if report["status"] == "completed":
                ...
                if output_path and Path(output_path).exists():
                    ...
                    st.caption(
                        "Unzip the download, then double-click **'Open in Power BI Desktop.bat'** "
                        "inside the extracted folder — it launches the project for you. "
                        "(Or manually open the .pbip file: File → Open → Power BI Project.)"
                    )
            else:
