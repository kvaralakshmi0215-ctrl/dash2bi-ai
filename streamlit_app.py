...
try:
    ...
    # No secrets.toml configured at all — fall back to the heuristic matcher.
    os.environ.setdefault("AI_PROVIDER", "none")
...
def reset_downstream(from_step: str):
    ...
    Uses strict `<` so `from_step`'s own just-computed results are preserved —
    e.g. reset_downstream("analyze") must clear mapping/preview/convert but
    NOT the analysis results it was just called after."""
